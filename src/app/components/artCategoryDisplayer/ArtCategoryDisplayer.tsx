  ...
  return (
    <div className="art-category" ref={scope}>
          ...
          return (
            <button
              ...
              className={`btn btn-colored ${
                colorOrderArray[index % colorOrderArray.length]
              }`}
              onClick={() => {
        ...
        <div className="confine">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="detail"
              key={"detail" + categoryList[activeCategory].title}
            >
          ...
          {categoryList[activeCategory].pricing && pricing > 0 && (
            <>
              <div className="pricings">
                <AnimatePresence>
                  {categoryList[activeCategory].pricing &&
                    categoryList[activeCategory].pricing?.map(
                      (price, index) => {
                        return (
                          <motion.div
                            initial={{ opacity: 0, x: -100 }}
      ...
      <section className="art-list">
        {renderer[categoryList[activeCategory].renderType](
          categoryList[activeCategory].imageList
        )}
      </section>
    </div>
  );
